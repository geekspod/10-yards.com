...

{
...

	{
		...
		$data['roles'] = $this->Model_roles->view_all_roles();
...

	{
		...
		if(isset($_POST['form2'])) {
			...
			$this->form_validation->set_rules('organization_name', 'Organization Name', 'trim|required');
				...
				$this->form_validation->set_rules('cities_name', 'Cities Name', 'trim|required');
    ...
    //         $path = $_FILES['banner']['name'];
		  ...
		  //  }
...

		    {
				...
				// $next_id = $this->Model_roles->get_auto_increment_id();
...
//echo "<pre>";print_r($form_data);exit;
	            $this->Model_roles->add_roles_data($form_data);

//echo "success";exit;

		        $success = 'roles are added successfully!';
...

		    {
				...
				redirect(base_url().'admin/roles/add');
            ...
            $data['organization'] = $this->Model_category->get_organization();
             $data['department'] = $this->Model_category->get_department();
               $data['position'] = $this->Model_category->get_position();
               $data['cities'] = $this->Model_category->get_cities();

            $this->load->view('admin/view_header',$data);

			$this->load->view('admin/view_roles_add',$data);
...
//echo "<pre>";print_r($tot);exit;
	...
	public function edit_roles_data($id)

{
    $id=$this->uri->segment(4);
   // echo "<pre>";print_r($id);exit;
	$tot = $this->Model_roles->roles_check($id);
	//echo "<pre>";print_r($tot);exit;
	if(!$tot) {
		redirect(base_url().'admin/roles');
		exit;
	...
	if(isset($_POST['form2'])) 
	{
		...
		if($valid == 1) 
		{
			$data['roles'] = $this->Model_roles->get_edit_roles($id);
			...
			if($data['roles']  == '') {
				...
				$this->Model_roles->roles_update($id,$form_data);
			}
			else {
				// unlink('./public/uploads/'.$data['category']['category_banner']);
		...
		//	echo "<pre>";print_r($form_data);exit;
			...
			$success = 'Roles are updated successfully';
			$this->session->set_flashdata('success',$success);
			redirect(base_url().'admin/roles');
		}
		else
		{
			$this->session->set_flashdata('error',$error);
			redirect(base_url().'admin/roles/edit/'.$id);
		}
	   
	} else {
// 		$sub_categories = $this->Model_category->get_edit_roles($id);
...
// 		$dimensions_id=$sub_categories['dimensions_id'];
	...
	//	$data['roles'] = $this->Model_category->get_edit_roles($id);
...
// 		$data['sub_categories']=$sub_categories;
		...
		$this->load->view('admin/view_header',$data);
		$this->load->view('admin/view_roles_edit',$data);
		$this->load->view('admin/view_footer');
	}
...
public function delete_personality_assessment($id){
...

	{
		...
		$categories = $this->Model_roles->get_roles($id);



		$this->Model_category->delete_roles($id);

		$success = 'Roles are deleted successfully';
...
public function add_description(){
	...
	$data['setting'] = $this->Model_common->get_setting_data();
		...
		if(isset($_POST['form4'])) {
			...
			$this->form_validation->set_rules('description_dimensions', 'Description Dimensions', 'trim|required');
...

		    {
		        ...
		        $form_data = array(
					...
					'description_dimensions'       => $_POST['description_dimensions'],
